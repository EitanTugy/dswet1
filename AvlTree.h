		...
		*/
		...
		Node(Node& node) {
			try {
				key = NULL;
				data = NULL;
				//std::cout << "cool"  << node.type  << "cool" << std::endl;
...
public:
	...
	*/
	...
	type(type) {
		...
		//std::cout << "type Array = " << array_of_nodes[0]->getType() << std::endl;
		//std::cout << "type Array = " << array_of_nodes[1]->getType() << std::endl;
		//(array_of_nodes[0]->getData())->print();
		//(array_of_nodes[1]->getData())->print();
